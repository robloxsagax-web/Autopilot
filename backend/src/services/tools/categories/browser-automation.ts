      ...
      switch (action) {
        ...
        case 'extract':
          ...
          if (extractType === 'markdown') {
            ...
            const result = await extractor.extractContent(page.url());
...
export const browserGetMarkdownTool = tool({
  description: 'Extract webpage content as clean markdown using readability algorithm',
  parameters: z.object({
    url: z.string().optional().describe('URL to extract content from (uses current page if not provided)'),
    maxPages: z.number().optional().default(1).describe('Maximum number of pages to process for paginated content'),
  }),
  execute: async ({ url, maxPages }) => {
    console.log(`📄 Extracting markdown content from: ${url || 'current page'}`);
